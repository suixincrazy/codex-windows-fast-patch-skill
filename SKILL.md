...
- For the supported Windows 10 screenshot-helper profiles, `scripts\patch-computer-use-helper-win10.ps1` reports `patched` with the selected profile's complete output SHA-256 and an original backup matching the profile's complete input SHA-256. The `0.4.20` and `0.5.2` profiles additionally have repeated-static resource checks and dynamic-capture image-change checks. Their documented pairs are `0.4.20`: `F2B2F56F...` -> `71A13CBC...` and `0.5.2`: `2C4CAC16...` -> `D816B14A...`. The `0.6.6` profile is documented with a cold/repeated-static capture check only: `BE488E66...` -> `34D6EB4F...`; dynamic-capture and long-run resource-stability validation remain pending for that profile.
